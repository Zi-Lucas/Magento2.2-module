...
class Pass extends \Aosom\Marketing\Controller\Adminhtml\Trial
{
        ...
        if ($id) {
            ...
            if (!empty($customerId)){
                ...
                if (!empty($to)){
                    ...
                    if ($helper->sendEmail('',$to,'',$helper->trialEmailTemplete('pass'))){
                        $this->messageManager->addSuccess(__('Send Pass Email Success.'));
    ...
    {
        return $this->_authorization->isAllowed('Aosom_Marketing::trial_pass');
    }
}
